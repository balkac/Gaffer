...
- **101 dotnet testi yeşil** — `dotnet test tests/Gaffer.Tests.csproj` (aynı testler Unity Test Runner'da da koşar)
...
- **Domain** — `Attributes` (gruplu ~29 alan: Teknik/Set-piece/Fiziksel&Hareket/Kalecilik), `RoleKeyAttributes` (rol→anahtar-attribute eşlemesi), `Player`, `PlayerId`, `Position` (Players); `TeamStrength`, `ClubId`, `Club` (opsiyonel `Squad` taşır), `Squad` (immutable + `Add`/`Remove`/`Contains`) (Clubs); `League` (Leagues)
...
- **Tests** — 101 test (`Gaffer.Tests`); `Gaffer.Tests.Unity` runtime determinizm kontrolü
...
- **Faz 3 (Yönetim, devam) — sıradaki:** ✅ `Squad` + `BuildEffectiveStrength` + `SquadGenerator` + `Club`↔`Squad` + gruplu attribute modeli + isimli golcü seçimi + **taktik** (`Tactics`) + **ilk 11 seçimi** (`Formation` + `LineupSelector`; güç sahadaki 11'den, Season Player'da formasyon + Start/Bench) hazır. ✅ **scout maskesi + değerleme + al/sat ekonomisi + maaşlar** (`Scout`/`PlayerValuation`/`PlayerWage`/`Finances`/`TransferService`, CM-sadık iki-bütçe; Transfer Market penceresi nakit+maaş+Sign/Sell ile gösteriyor) hazır → keşfet-**sat** döngüsü kapandı. **Sıradaki (kullanıcı önceliği):** **granüler mevki** (sağ bek/stoper/kanat… → formasyon slotları + üretim + ilk-11 seçimi gerçekten farklılaşır; GK/DEF/MID/FWD geniş hattı sim için kalır). Sonra oyuncu **gelişimi** (potansiyel zamanla realize → "büyüt", flip'in eksik parçası) → `ClubGenerator`/isim üreteci → `BalanceSO`. *(Save'de taktik/dizilim/ekonomi kalıcılığı sonraya.)*
