...
- **80 dotnet testi yeşil** — `dotnet test tests/Gaffer.Tests.csproj` (aynı testler Unity Test Runner'da da koşar)
...
- **Application/Season** — `FixtureScheduler` (çift devre, circle method), `LeagueTable`, `LeagueSeason` (hafta ilerlet + sonuç geçmişi + `Restore`; her kadrolu kulübün gücünü **ilk 11 + taktikten** her hafta türetir — `SetStarters`/`SetFormation`/`SetTactics`), `MatchResult` (skor + dakika-golleri), `BoardTarget` + `SeasonEvaluator` → `SeasonVerdict`
...
- **Tests** — 80 test (`Gaffer.Tests`); `Gaffer.Tests.Unity` runtime determinizm kontrolü
