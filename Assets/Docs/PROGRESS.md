...
- **Editor (`Gaffer.Editor`)** — `SeasonHarnessWindow` (dağılım/Gate A workbench) + `SeasonPlayerWindow` (bir sezonu izle: tablo + roster/ilk-11 + taktik + isimli golcüler + şut + verdict) + `TransferMarketWindow` (nakit + maaş bütçesi + baz kadro + market: scout-maskesiyle gez, doğruluk slider'ıyla aralıklar daralır, **Sign/Sell** ile al-sat, maaş bordrosu/yıllık maliyet görünür)
